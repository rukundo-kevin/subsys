...
export const submissionNotification = async (user: User, students: StudentAssignment[]) => {
  try {
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
    ...
    const mailOptions = {
      ...
      <div style="margin-left:5rem;margin-bottom:5rem">
      <h4 style="margin-bottom:8px;color:#000000;">${students.join(' ')}</h4>
      <p style="color:#000000;">We would like to invite to review and evaluate recent assignment submissions made by some of our students.</p>
      </div>
      ...
      <table width="100%" height="100%" style="display:table">
          ...
          <div style="border-radius: 10px;width:200px;height:40px;background-color:#5D34EC; margin: auto;">
              <a href='${
                config.frontendUrl
              }submissions/' style="color:#FFFFFF;text-decoration: none;font-size: 20px;display:block; text-align: center; line-height: 40px;">View submisssions</a>
           </div>
          </td>
          </tr>
         </table>
